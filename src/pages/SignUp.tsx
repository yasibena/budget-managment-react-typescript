...
export const SignUp = () => {
    ...
    const handleSumbit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        if (!termsAgreed || username.trim().length === 0 || email.trim().length === 0 || password.trim().length === 0) {
            setError(true)
            return;
        }
        try {
                ...
                <Box component="form" bgcolor='#1d1933' mb='0' border='2px solid #2f2b43' borderRadius='5px' p={3} onClick={(event) => handleSumbit(event)}>
                    <Grid2 container spacing={2} flexDirection='column' alignItems='center' justifyContent='space-around'>
                        ...
                        <Grid2 item xs={12} sm={6} width='100%'>
                            ...
                            />
                            {
                                (username.trim().length == 0 && showALert) &&
                                <Alert severity="error" sx={{ bgcolor: 'transparent', p: '0' }}>Please type username.</Alert>
                            }
                        </Grid2>
                        <Grid2 item xs={12} sm={6} width='100%'>
                            <TextField
                        ...
                        <Grid2 item xs={12} width='100%'>
                            <Button sx={{
                                ...
                                {loadingSignUp ? <CircularProgress /> : 'Sign up'}
                            </Button>
                        </Grid2>
                        <Typography color='#aea9e4'>Already have an account?
                            <Link to='/' style={{ textDecoration: 'none' }} >
                                <span style={{ color: '#AD49E1', fontSize: 'bold', cursor: 'pointer' }}> Log in</span>
                            </Link>
                        </Typography>
                    </Grid2>
                </Box>
